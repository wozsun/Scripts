...
- 路径必须是 Windows 文件或文件夹绝对路径，例如 `C:\Users\Name\Documents` 或 `C:\Users\Name\Documents\A.doc`；交互输入时可以粘贴带首尾英文引号的路径。
...
- 路径必须是 Windows 文件夹绝对路径；交互输入可分行，也可在同一行用空格或英文分号分隔，路径含空格请使用英文引号。
...
- 路径必须是 Windows 文件夹绝对路径；交互输入可分行，也可在同一行用空格或英文分号分隔，路径含空格请使用英文引号。
- 输入目录不能重复，也不能互相包含，避免同一目录在一次运行中被重复扫描或处理。
...
- 删除使用 `Remove-Item`，不会进入回收站；扫描、判断或删除失败时会提示并继续处理后续目录。
